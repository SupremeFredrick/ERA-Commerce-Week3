...
app.get("/products/:id", authenticateToken, async (req, res) => {
    const { id } = req.params;
    const sql =
        "SELECT p.id, p.name, p.description, p.price, p.stock_quantity, c.name AS category_name FROM products p INNER JOIN categories c ON p.category_id = c.id WHERE p.id = ?";
...
app.post("/orders", authenticateToken, async (req, res) => {
    ...
    db.beginTransaction(async (err, result) => {
        if (err) return res.status(500).json({ message: "Server error." });
        try {
...
app.get("/orders/:id", authenticateToken, (req, res) => {
    const { id } = req.params;
    const sql =
        "SELECT o.id AS order_id, o.status, o.total_amount, o.created_at, oi.id, oi.quantity, oi.price_at_purchase, oi.subtotal, p.name AS product_name FROM orders o INNER JOIN order_items oi ON oi.order_id = o.id INNER JOIN products p ON p.id = oi.product_id WHERE o.id = ? ORDER BY oi.id ASC";
...
app.post("/reviews", authenticateToken, async (req, res) => {
    ...
    if (!product_id || !rating || !review_text) {
        return res.status(400).json({ message: "Product id, Rating, & Review text are required" });
    }
